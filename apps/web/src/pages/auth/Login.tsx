...
// Demo accounts — the real backend has no email/password login endpoint (it verifies OIDC
// bearer tokens only, see apps/api/src/identity/oidc.ts), so this screen intentionally stays a
// local mock. Once the family is created/joined, the app switches to using the real inventory
// and shopping endpoints with the resulting familyId — see App.tsx.
    ...
    <AuthShell title="Bentornato" subtitle="Accedi al tuo account per continuare">
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div>
          ...
          <Input type="email" value={email} onChange={(e) => { setEmail(e.target.value); setState("IDLE"); }} placeholder="nome@esempio.it" autoComplete="email" required />
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <Input type={showPw ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setState("IDLE"); }} placeholder="••••••••" autoComplete="current-password" required style={{ paddingRight: "3.5rem" }} />
            <button
              type="button"
        ...
        <button
          ...
          style={{ backgroundColor: state === "SUBMITTING" ? colors.disabled : colors.terracotta, color: colors.white }}
        >
      ...
      <div className="pt-2 text-center">
        ...
        <button onClick={onRegister} className="text-xs font-semibold transition-opacity hover:opacity-70" style={{ color: colors.terracotta }}>
          Registrati
        </button>
      </div>
      ...
      <div className="rounded-xl p-3 space-y-1.5" style={{ backgroundColor: colors.cream, border: `1px solid ${colors.border}` }}>
        <p className="text-[10px] font-semibold" style={{ color: colors.inkMuted }}>Account demo (password: password123)</p>
        <div className="space-y-0.5">
          {Object.values(DEMO_ACCOUNTS).map((a) => (
            <button
              key={a.email}
              onClick={() => { setEmail(a.email); setPassword("password123"); setState("IDLE"); }}
              className="w-full text-left text-[10px] px-2 py-1 rounded-lg transition-colors hover:opacity-70"
              style={{ color: colors.inkMuted }}
            >
              <span className="font-medium" style={{ color: colors.ink }}>{a.email}</span>
              {" "}· {a.role}{a.hasFamilyId ? "" : " · nessuna famiglia"}
            </button>
          ))}
        </div>
      </div>
